...
import { ShiftAttributes, ShiftValues } from "timesheets/types";
...
export const expectValidShiftAttributes = (
  shiftAttributes: ShiftAttributes
) => {
  expect(typeof shiftAttributes.start).toBe("string");
  expect(typeof shiftAttributes.end).toBe("string");
  expect(typeof shiftAttributes.breakDuration).toBe("number");
};
...
describe("calls timesheet submit handler when submit button clicked", () => {
  test("with all shifts active", (done) => {
    ...
    render(
      <TimesheetForm
        defaultShiftValues={testShifts}
        onSubmitTimesheet={({ shiftsAttributes }) => {
          expect(shiftsAttributes.length).toEqual(testShifts.length);
          shiftsAttributes.forEach((shiftAttributes) =>
            expectValidShiftAttributes(shiftAttributes)
          );
          done();
        }}
        onSubmitDefaultShiftValues={noop}
  ...
  test("with only some shifts active", (done) => {
    ...
    render(
      <TimesheetForm
        defaultShiftValues={testShifts}
        onSubmitTimesheet={({ shiftsAttributes: shifts }) => {
          expect(shifts.length).toEqual(testShifts.length - 1);
          shifts.forEach((shift) => expectValidShiftAttributes(shift));
          done();
        }}
        onSubmitDefaultShiftValues={noop}
      />
